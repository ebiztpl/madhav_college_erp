...
class Incomehead_model extends My_Model
{
    ...
    public function addcategory($data)
    {
        ...
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('income_head_category', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  income head category  id " . $data['id'];
            $action    = "Update";
            $record_id = $return_value = $data['id'];
        } else {
            $this->db->insert('income_head_category', $data);
            $return_value = $this->db->insert_id();
            $message      = INSERT_RECORD_CONSTANT . " On  income head category  id " . $return_value;
            $action       = "Insert";
            $record_id    = $return_value;
        }
        $this->log($message, $record_id, $action);
    ...
    public function getcategory($id = null){
        $this->db->select()->from('income_head_category');
        if ($id != null) {
            $this->db->where('id', $id);
        } else {
            $this->db->order_by('id');
    ...
    public function removecategory($id)
    {
        ...
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('income_head_category'); 
               
        $message   = DELETE_RECORD_CONSTANT . " On  income head category  id " . $id;
        $action    = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
