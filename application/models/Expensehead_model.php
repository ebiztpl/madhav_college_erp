...
class Expensehead_model extends MY_Model
{
    ...
    public function addcategory($data)
    {
        ...
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('expense_head_category', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  expense head category id " . $data['id'];
            $action    = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $this->db->insert('expense_head_category', $data);
            $id        = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On  expense head category id " . $id;
            $action    = "Insert";
            $record_id = $id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
    ...
    {
        $this->db->select()->from('expense_head_category');
        if ($id != null) {
            $this->db->where('id', $id);
        } else {
            $this->db->order_by('id');
    ...
    public function removecategory($id)
    {
        ...
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('expense_head_category');
        $message   = DELETE_RECORD_CONSTANT . " On  expense head category id " . $id;
        $action    = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
