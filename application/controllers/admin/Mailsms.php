...
//==============
    ...
    public function whatsapp_template()
    {
        ...
        $sms_template_list         = $this->messages_model->get_whatsapp_template();
        $data['sms_template_list'] = $sms_template_list;
        $this->load->view('layout/header');
    ...
    public function add_whatsapp_template()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                'title'      => $this->input->post('title'),
                'template_name'    => $this->input->post('message'),
    ...
    public function delete_whatsapp_template($id)
    {
        ...
        $this->session->set_flashdata('message', $this->lang->line('delete_message'));
        redirect('admin/mailsms/whatsapp_template/whatsapp_template');
    }
    ...
    public function edit_whatsapp_template()
    {
        ...
        $id                        = $this->input->post('id');
        $data['sms_template_list'] = $this->messages_model->get_whatsapp_template($id);
// print_r($data['sms_template_list']);die;
    ...
    public function compose_whatsapp()
    {
        ...
        $data['title']             = 'Add Whatsappsms';
        $class                     = $this->class_model->get();
        $data['classlist']         = $class;
        $data['sms_template_list'] = $this->messages_model->get_whatsapp_template();
        $userdata                  = $this->customlib->getUserData();
        $carray                    = array();
        $date                      = date('Y-m-d');
        $birthDaysList             = array();
    ...
    {
        $template_id       = $this->input->post('template_id');
        $sms_template_list = $this->messages_model->get_whatsapp_template($template_id);
        echo json_encode(array('data' => $sms_template_list));
    }
            ...
            if ($send_type == 'send_now') {
                if (!empty($user_array)) {
                    ...
                    foreach ($user_array as $user_mail_key => $user_mail_value) {
                        if (in_array("sms", $sms_mail)) {
                            if ($user_mail_value['mobileno'] != "") {
                                $this->sendwhatsapp($user_mail_value['mobileno'], $template_id);
                            }
                        }
                        if (in_array("push", $sms_mail)) {
                            $push_array = array(
    ...
    // public function sendwhatsapp($mobile,$tempid){


    //     $this->db->select('*')->from('whatsapp_template');
    //     $this->db->where('whatsapp_template.id', $tempid);
    //     $query = $this->db->get();
    //    $dat = $query->row();
   
    

    // $url = "https://graph.facebook.com/v15.0/103063532495982/messages";
    //     $curl = curl_init($url);
    //     curl_setopt($curl,CURLOPT_URL,$url);
    //     curl_setopt($curl,CURLOPT_POST,true);
    //     curl_setopt($curl,CURLOPT_RETURNTRANSFER,true);
    //    $headers = array(
    //        "Authorization: Bearer EAALxnOpZByg0BO0ciAHGmVeukIT93sJ0N57Dcp8eLzrRLfVtjG27tNgx2A2RXmeqo3p2NoZBJ9iEdBeZBKPXVOZAxH5aYAgXZAHED6HHsTGyqqU99q90ZAPqGapUTMEXCsN6VZCLuCq8xeSVbfHqsbX5ZAErJNrBUZCKfcp02TxBIxNHsPNbGE6mpb8aGQXxFUHlgWlUonTZADnBiZC7sL0",
        ...
        if ($this->form_validation->run()) {
            ...
            if (!empty($user_array)) {
                foreach ($user_array as $user_mail_key => $user_mail_value) {
                    if (in_array("sms", $sms_mail)) {
                        if ($user_mail_value['mobileno'] != "" && $user_mail_value['mobileno'] != 0) {
                            $this->sendwhatsapp($user_mail_value['mobileno'], ($message), $template_id, "");
                        }
                    }
                }
            }
