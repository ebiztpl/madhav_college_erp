...
class Income_model extends My_Model
{
    ...
    {
        $this->db->select('income.id,income.date,income.session,income.name,income.invoice_no,income.amount,income.documents,income.note,income_head.income_category,income.income_head_id')->from('income');
        $this->db->join('income_head', 'income.income_head_id = income_head.id');
        if ($id != null) {
