...
class Certificate extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('background_image', $this->lang->line('image'), 'required|callback_handle_upload');
    ...
    public function set_positions($id)
    {
        if (!$this->rbac->hasPrivilege('student_certificate', 'can_view')) {
            access_denied();
        }
        ...
        $this->session->set_userdata('top_menu', 'Certificate');
        $this->session->set_userdata('sub_menu', 'admin/set_positions');

    
       if (isset($_GET["gt"])) {
       $b =  $_GET["gt"];
        $this->db->where('certificate_id',$id);
        $this->db->delete('certificate_layout');
      }


        $feilds = $this->student_model->getStudentsByArray([3919]);
        
      
        $this->db->select()->from('certificates');
        $this->db->where('id', $id);
        $query =  $this->db->get();
        $certificate = $query->row();
        ...
        $this->data['certificatedata'] =  $certificatedata;


        $this->data['certificate'] =  $certificate;
        $this->data['feilds'] =  $feilds[0];
        $this->load->view('layout/header');
        $this->load->view('admin/certificate/setcertificate', $this->data);
        $this->load->view('layout/footer');
        
    }


    public function savecertificatelayout()
    {
...
if($certificatedata){
    $array = array(
       ...
       $this->db->where('certificate_id',$certificateid);
       $this->db->update('certificate_layout',$array);
}else{
    $array = array(
        'certificate_id' => $certificateid,
        'data' =>$feilds,
       ...
       $this->db->insert('certificate_layout',$array);
}
