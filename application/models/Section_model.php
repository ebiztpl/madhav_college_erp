...
class Section_model extends MY_Model {
    ...
    public function getCityByState($classid) {
        $userdata = $this->customlib->getUserData();
        $role_id = $userdata["role_id"];
        $carray = array();
        ...
        if (isset($role_id) && ($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
            ...
            $this->db->join('ci_states', 'ci_cities.state_id = ci_states.id');
            $this->db->where('ci_cities.state_id', $classid);
            $this->db->order_by('ci_cities.name');
            $query = $this->db->get();
            $section = $query->result_array();
        }
