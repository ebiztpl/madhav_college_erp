...
// print_r($data);die;
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('addhar_no', "Adhar", 'trim|required|xss_clean');
        $this->form_validation->set_rules('pan_no',"Pan ", 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($note)) {
                $data_insert['addhar_no'] = $this->input->post("addhar_no");
            }
            if (isset($note)) {
                $data_insert['pan_no'] = $this->input->post("pan_no");
            }
            if (isset($note)) {
                $data_insert['nominee'] = $this->input->post("nominee");
            }
            if (isset($note)) {
                $data_insert['nominee_number'] = $this->input->post("nominee_number");
            }
            ...
            if ($date_of_leaving != "") {
                $data_insert['date_of_leaving'] = $this->customlib->dateFormatToYYYYMMDD($date_of_leaving);
            }else{
...
//==========================
            ...
            if ($insert) {
              ...
              $insert_id =  $this->staff_model->add($data_doc);
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data1 = array(
                ...
                'nominee_number'    =>$this->input->post('nominee_number'),
                'surname'              => $surname,
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result as $r_key => $r_value) {
                            ...
                            if ($check_exists == 0 && $check_emailexists == 0) {
                                ...
                                $result[$r_key]['addhar_no']                 = $this->encoding_lib->toUTF8($result[$r_key]['addhar_no']);
                                $result[$r_key]['pan_no']                 = $this->encoding_lib->toUTF8($result[$r_key]['pan_no']);
                                $result[$r_key]['nominee']                 = $this->encoding_lib->toUTF8($result[$r_key]['nominee']);
                                $result[$r_key]['nominee_number']                 = $this->encoding_lib->toUTF8($result[$r_key]['nominee_number']);
                                ...
                                $result[$r_key]['cpf_no']               = $this->encoding_lib->toUTF8($result[$r_key]['cpf_no']);
