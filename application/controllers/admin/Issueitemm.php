...
class Issueitemm extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'issueitemm/index');
        $data['title']      = 'Add Issue item';
        $data['title_list'] = 'Recent Issue items';
        $this->load->view('layout/header', $data);
        $this->load->view('admin/issueitemm/issueitemList', $data);
    ...
    public function create()
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/issueitemm/issueitemCreate', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->itemissuee_model->add($data);
    ...
    public function check_available_quantity()
    {
        ...
        if ($quantity != "" && $item_category_id != "" && $item_id != "") {
            ...
            $data      = $this->itemm_model->getItemAvailable($item_id);
    ...
    {
        $data['title'] = 'Delete';
        $this->itemissuee_model->remove($id);
        redirect('admin/issueitemm');
    }
    ...
    public function getitemlist()
    {
        ...
        $m               = $this->itemissuee_model->getitemlist();
        $m               = json_decode($m);
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('issue_item', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . "  )' href='" . base_url() . "admin/issueitemm/delete/" . $value->id . "' class='btn btn-default btn-xs' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-remove'></i></a>";
                }
