...
class Itemstockk extends Admin_Controller
{
    ...
    {
        if (!$this->rbac->hasPrivilege('item_stockk', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Inventory');
        $this->session->set_userdata('sub_menu', 'Itemstockk/index');
        $data['title']      = 'Add Item';
        ...
        $this->form_validation->set_rules('depreciation_value', 'Depreciation Value', 'trim|numeric|xss_clean');
        $this->form_validation->set_rules('depreciation_amortisation', 'Depreciation Amortisation', 'trim|numeric|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data     = array(
                ...
                'attachment'     => $img_name,
                'depreciation_value'    => $this->input->post('depreciation_value'),
                'depreciation_amortisation'    => $this->input->post('depreciation_amortisation'),
                'session'    => $this->input->post('popup_session'),
                
            );
            $insert_id = $this->itemstockk_model->add($data);         

            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/itemstockk/index');
        }
        $item_result = $this->itemstockk_model->get();

        $data['itemlist']     = $item_result;
        $itemcategory         = $this->itemcategoryy_model->get();
        $data['itemcatlist']  = $itemcategory;
        $itemsupplier         = $this->itemsupplierr_model->get();
        $data['itemsupplier'] = $itemsupplier;
        $itemstore            = $this->itemstoree_model->get();
        $data['itemstore']    = $itemstore;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/itemstockk/itemList', $data);
        $this->load->view('layout/footer', $data);
    }

    public function download($id)
    {
        $itemlist = $this->itemstockk_model->get($id);
    ...
    public function getItemByCategory()
    {
        ...
        $data             = $this->itemm_model->getItemByCategory($item_category_id);
    ...
    public function getItemunit()
    {
        ...
        $data = $this->itemm_model->getItemunit($id);
        echo json_encode($data);
    }
    ...
    public function edit($id)
    {
        ...
        $data['id']           = $id;
        $item                 = $this->itemstockk_model->get($id);
        $data['item']         = $item;
        ...
        $itemunitdata      = $this->itemm_model->getItemunit($item['item_id']);
        ...
        $this->form_validation->set_rules('depreciation_amortisation', 'Depreciation Amortisation', 'trim|numeric|xss_clean');
          if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/itemstockk/itemEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $store_id = ($this->input->post('store_id')) ? $this->input->post('store_id') : null;
            $data     = array(
            ...
            $this->itemstockk_model->add($data);         
