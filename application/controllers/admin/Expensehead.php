...
class Expensehead extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $data['categorylistt'] = $query->result_array();
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('exp_head_id', 'Expense Head category', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'head_category' => $this->input->post('exp_head_id'),
    ...
    public function expenseheadcategory()
    {
        ...
        $this->session->set_userdata('top_menu', 'Expenses');
        $this->session->set_userdata('sub_menu', 'expenseshead/expenseheadcategory');
        $data['title'] = 'Expense Head category List';

        // $category_result      = $this->expensehead_model->getcategory();
        // $data['categorylist'] = $category_result;
        ...
        $this->account->order_by('id', 'desc');
        $query = $this->account->get();
        $data['categorylist'] = $query->result_array();


        $this->load->view('layout/header', $data);
        $this->load->view('admin/expensehead/expenseheadcategoryList', $data);
        $this->load->view('layout/footer', $data);
    }


    public function expenseheadcategorycreate()
    {
        if (!$this->rbac->hasPrivilege('expense_head', 'can_add')) {
            access_denied();
        }
        ...
        $category_result      = $this->expensehead_model->getcategory();
        $data['categorylist'] = $category_result;
        $this->form_validation->set_rules('expensehead','Expense Head Category', 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/expensehead/expenseheadcategoryList', $data);
            $this->load->view('layout/footer', $data);
            ...
            $data = array(
                'title' => $this->input->post('expensehead'),
                'description'  => $this->input->post('description'),
            );
            $this->expensehead_model->addcategory($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . $this->lang->line('success_message') . '</div>');
            redirect('admin/expensehead/expenseheadcategory');
        }
    }
    ...
    public function deletecategory($id)
    {
        ...
        $data['title'] = 'Expense Head List';
        $this->expensehead_model->removecategory($id);
    ...
    public function editcategory($id)
    {
        ...
        $data['title']        = 'Edit Expense Head Category';
        $category_result      = $this->expensehead_model->getcategory();
        $data['categorylist'] = $category_result;
        $data['id']           = $id;
        $category             = $this->expensehead_model->getcategory($id);
        $data['expensehead']  = $category;
        $this->form_validation->set_rules('expensehead', 'Expense Head Category', 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/expensehead/expenseheadcategoryEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'id'           => $id,
