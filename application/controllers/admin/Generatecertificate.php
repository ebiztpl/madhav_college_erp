...
class Generatecertificate extends Admin_Controller
{
    ...
    public function generatemultiple()
    {
        ...
        // $data['sch_setting'] = $this->sch_setting_detail;
        // if($data['certificate']['0']->id == 3){
        //     $certificates        =      $this->load->view('admin/certificate/printcertificatetetc', $data, true);
        // }elseif($data['certificate']['0']->id == 4){
        //     $certificates        =     $this->load->view('admin/certificate/printcertificatecc', $data, true);
        // }elseif($data['certificate']['0']->id == 5){
        //     $certificates        =     $this->load->view('admin/certificate/printcertificateregular', $data, true);
        // }elseif($data['certificate']['0']->id == 6){
        //     $certificates        =     $this->load->view('admin/certificate/printprepresentiationcertificate', $data, true);
        // }elseif($data['certificate']['0']->id == 7){
        //     $certificates        =     $this->load->view('admin/certificate/printnoduecertificate', $data, true);
        // }else{
        //     $certificates        =      $this->load->view('admin/certificate/printcertificate', $data, true);
        // }
        // echo $certificates;
        $data['students'] = $this->student_model->testgetStudentsByArray($std_arr);
        foreach ($data['students'] as $key => $value) {
            $data['students'][$key]['name'] = $this->customlib->getFullName($value['firstname'], $value['middlename'], $value['lastname'], $this->sch_setting_detail->middlename, $this->sch_setting_detail->lastname);
        }
        ...
        $data['certificatedata'] = $certificatedata;
            ...
            $certificates        =      $this->load->view('admin/certificate/finalcertificategenerate', $data, true);
      
        echo $certificates;
    }


    public function generatemultiplenew($id)
    {
        ...
        $data['students'] = $this->student_model->testgetStudentsByArray([421]);
        foreach ($data['students'] as $key => $value) {
            $data['students'][$key]['name'] = $this->customlib->getFullName($value['firstname'], $value['middlename'], $value['lastname'], $this->sch_setting_detail->middlename, $this->sch_setting_detail->lastname);
        }
        ...
        $data['certificatedata'] = $certificatedata;
            ...
            $certificates        =      $this->load->view('admin/certificate/testgenerate', $data, true);
    ...
    public function saveissueinformation()
    {
        $certificate_id     = $this->input->post('certificate_id');
        $studentid     = $this->input->post('student_id');
        $certificate_issue_date     = date("Y-m-d", strtotime(str_replace("/","-",$this->input->post('certificate_issue_date')))) ;;
        $certificate_remark     = $this->input->post('certificate_remark');


        $this->db->insert('certificate_issue_details', array( 'certificate_id'	 =>$certificate_id , 'studentid'	 =>$studentid ,  'certificate_issue_date'	 =>$certificate_issue_date,  'certificate_remark'	 =>$certificate_remark ));
  echo 1;
    }
