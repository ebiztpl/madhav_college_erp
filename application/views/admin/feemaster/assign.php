    ...
    <section class="content">
        <div class="row">
            <div class="col-md-12">
                ...
                $this->db->where('fee_groups.id', $feegroupList[0]->fee_groups_id);
                $queryy = $this->db->get();
                $res = $queryy->row(); 
                
                $classarray = json_decode($res->courses) ;
                ...
                ?>
                    ...
                    <div class="box-header with-border">
                        <h3 class="box-title"><i class="fa fa-search"></i> <?php echo $this->lang->line('select_criteria'); ?> / <span style="color:red"><?=  $feegroupList[0]->group_name ?></span></h3>
                        
                    
                      
                        <!-- <div class="box-tools pull-left">
                                        <b style="color:red">Total Records Fetched:</b>                                   </div>
                    </div> -->
                            ...
                            <div class="">
                                <div class="box-header with-border">
                                    ...
                                    <div class="box-tools pull-right">
                                        <b>Total Students Fetched:</b> <?=count($resultlist)?>
                                    </div>
                                </div>
                                <div class="box-body">
                                    <div class="row">
                                        <div class="">
                                            <div class="col-md-4">
                                                <div class="table-responsive">
                                                    ...
                                                    foreach ($feegroupList as $feegroup) {
                                                        ...
                                                        <table class="table">
                                                            <thead>
                                                                <th><?php echo $this->lang->line('fees_code');?><label class="labelbold" style="margin-left:2%"><input type="checkbox" id="select_alll" autocomplete="off"> All</label></th>
                                                            ...
                                                            <tbody>
                                                                ...
                                                                foreach ($feegroup->feetypes as $feetype_key => $feetype_value) {
                                                                    ...
                                                                    <tr class="mailbox-name">
                                                                        ...
                                                                        <input class="checkboxx" type="checkbox" name="category_head_id[]"  value="<?php echo $feetype_value->id; ?>"/>
                                                                            <div style="margin-left: 2%;"><?php echo $feetype_value->code; ?></div>   </span>
                                                                        </td>
                                                                        <td class="text-right">
                                            ...
                                            <div class="col-md-8">
                                                                ...
                                                                foreach ($resultlist as $student) {
                                                                    ...
                                                                    <tr>
                                                                        <td> 
                                                                            ...
                                                                            $this->db->where('student_fees_master_head.fee_master_id', $student['student_fees_master_id']);
                                                                        ...
                                                                        <?php if($sel != ""){ ?>
                                                                        <a id="<?= $student['student_fees_master_id']?>" data-id="<?= $feegroup->group_name ?>"  class="btn btn-default btn-xs openmodal" data-toggle="tooltip" title="" data-original-title="View"><i class="fa fa-reorder"></i></a>
                                                                        <?php } ?>
                                                                    </td>
                                                                    </tr>
                                                                    <?php
                                                ...
                                                <button type="submit" class="allot-fees btn btn-primary btn-sm pull-right" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?php echo $this->lang->line('please_wait'); ?>">Apply
                                                </button>
                                                <?php } ?>
                                                <br/>
                                                <br/>
...
<div id="myModalbyrirtik" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div style="display: flex; justify-content: space-between;"><h4 class="modal-title">Applied Fee Codes</h4> <h4 class="modal-title">Pending Fee Codes</h4></div>
      </div>
...
//select all checkboxes
$("#select_alll").change(function () {  //"select all" change 
...
//".checkbox" change 
    $('.checkboxx').change(function () {
        //uncheck "select all", if one of the listed checkbox item is unchecked
        if (false == $(this).prop("checked")) { //if this item is unchecked
            $("#select_alll").prop('checked', false); //change "select all" checked status to false
        }
        //check "select all" if all checkbox items are checked
        if ($('.checkboxx:checked').length == $('.checkboxx').length) {
            $("#select_alll").prop('checked', true);
        }
    });
...
<script>
 ...
 $.ajax({
                type: "POST", 
                url: base_url + "studentfee/getappliedhead",
                data: {'fee_master_id': id , 'groupid' : groupid},
                success: function (response)
                {
                    $('#detaildata').empty();
                    $('#detaildata').append(response);
                    $('#myModalbyrirtik').modal('show');
...
<script>
  $(document).on('click', '.deleteappliedhead', function () {
    if (confirm('<?php echo $this->lang->line('are_you_sure'); ?>')) {
    $this = $(this);
    var base_url = '<?php echo base_url() ?>';
   var id = $(this).attr('id');
 ...
 $.ajax({
                type: "POST", 
                url: base_url + "studentfee/deleteappliedhead",
                data: {'id': id},
                success: function (response)
                {
                    // $('#detaildata').empty();
                    ...
                    alert(response);
                    if(response  == 'Fee Code Deleted Successfully'){
...
<script>
  $(document).on('click', '.addappliedhead', function () {
    $this = $(this);
    var base_url = '<?php echo base_url() ?>';
   var id = $(this).attr('id');
 ...
 $.ajax({
                type: "POST", 
                url: base_url + "studentfee/addappliedhead",
                data: {'id': id ,'headid':headid},
                success: function (response)
                {
                    // $('#detaildata').empty();
                    // $('#detaildata').append(response);
                    ...
                    //  $this.closest('.row').empty();
                     $this.closest('.row').html("<div class='col-md-12 text-center' style='color:green'>Fees Code Applied</div><br><br>");
                    $this.button('reset');
