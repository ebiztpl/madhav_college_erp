...
class Studenttransportfee_model extends MY_Model
{
    ...
    public function updatee($data_insert, $student_session_id)
    {
        ...
        if (!empty($data_insert)) {
            ...
            foreach ($data_insert as $insert_key => $insert_value) {
                # code...
                $this->db->where('student_session_id', $student_session_id);
                $this->db->where('subject', $insert_value['subject']);
                $q = $this->db->get('student_subjects');

                if ($q->num_rows() > 0) {
                    $row_insert[] = $q->row()->id;
                } else {

                    $this->db->insert('student_subjects', $data_insert[$insert_key]);
                    $row_insert[] = $this->db->insert_id();
                }
            }
            $this->db->where('student_session_id', $student_session_id);
    ...
    public function addd($data_insert, $student_session_id)
    {
        ...
        if (!empty($data_insert)) {
            foreach ($data_insert as $insert_key => $insert_value) {
                $this->db->insert('student_subjects', $insert_value);
    ...
    public function getsubjectsByStudentSession($student_session_id)
    {

        $this->db->select('subject')->from('student_subjects');
        $this->db->where('student_session_id', $student_session_id);
        $this->db->order_by('student_subjects.id', 'desc');
        $check2 = $this->db->get();
