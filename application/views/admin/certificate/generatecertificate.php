...
if (isset($resultlist)) {
    ?>
                        <form method="post" action="<?php echo base_url('admin/generatecertificate/generatemultiple') ?>">
                            <div  class="" id="duefee">
                                <div class="box-header ptbnull" style="color:red">Kindly set scale according to the certificate size while printing</div>
...
if (empty($resultlist)) {
        ...
        foreach ($resultlist as $student) {
            ...
            $this->db->where('certificate_id', $certificateResult[0]->id);
            $this->db->where('studentid', $student['id']);
            $query =  $this->db->get();
            $count_issue = $query->num_rows();
            ...
            ?>
                                                        <tr>
                                                            ...
                                                            <td><a class="btn btn-<?php if($count_issue == 0){ echo "success";  }else{   echo "primary"; }    ?> btn-xs saveissue" data-id="<?php echo $student['id']; ?>" id="<?= set_value('certificate_id') ?>"><?php if($count_issue == 0){ echo "Issue";  }else{   echo "Reissue"; }    ?></a></td>
                                                        </tr>
                                                        <?php
$count++;
        }
...
<div id="issuedatasave" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog">
    <form  method="post" class="saveissueinformation">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                        ...
                        <input type="text" value="<?php echo set_value('certificate_issue_date', date($this->customlib->getSchoolDateFormat())); ?>" class="form-control datee" name="certificate_issue_date" id="certificate_issue_date" required>
                        <span class="text-danger" id="certificate_issue_date"></span>
                    </div>
                    <input type="hidden" name="student_id" value="" id="student_id">

                <input type="hidden" name="certificate_id" value="<?php echo $certificateResult[0]->id ?>" id="certificate_id">
                <div class="form-group">
                                    <label>Remark</label><small class="req"> </small>
                                    <textarea autofocus="" id="certificate_remark" name="certificate_remark"  type="text" class="form-control" ></textarea>
            ...
            <button  type="button"  id="bfksgzhf"  class="btn btn-info">Save</button>
...
<script>
$(document).on('click', '#bfksgzhf', function (e){
    
    e.preventDefault(); 

    $.ajax({
                    url: '<?php echo site_url("admin/generatecertificate/saveissueinformation") ?>',
                    type: 'post',
                    dataType: "json",
                    data: $('.saveissueinformation').serializeArray(),
                    success: function (response) {
